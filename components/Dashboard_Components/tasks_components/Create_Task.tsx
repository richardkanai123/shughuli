...
const CreateTask = ({ projects }: { projects: projectProps[] }) => {
    ...
    const targetProject = projects.find((project) => project.slug === targetProjectid || project.id === targetProjectid)?.id
    const targetProjectDueDate = projects.find((project) => project.slug === targetProjectid || project.id === targetProjectid)?.dueDate
    ...
    const onSubmit: SubmitHandler<z.infer<typeof newTaskSchema>> = async (values) => {
        // check due date
        const isValidDueDate = isWithinInterval(new Date(values.dueDate), { start: new Date(), end: new Date(values.dueDate) })
        if (!isValidDueDate) {
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 p-6 rounded-xl shadow-lg transition-all duration-200 hover:shadow-xl">
                                ...
                                >
                                    <FormItem>
                                        ...
                                        <FormDescription className="text-sm opacity-80">
                                            Currently selected project: {form.getValues('projectId') ? projects.find((project) => project.id === form.getValues('projectId'))?.slug : 'None'}
                                        </FormDescription>
                                        <FormMessage />
                                    </FormItem>
                                </motion.div>
                                            ...
                                            <Popover>
                                                ...
                                                <PopoverContent className="w-auto p-0" align="start">
                                                    <Calendar
                                                        ...
                                                        disabled={(date) => !isDateValid(date, selectedProject?.dueDate)}
                                                        initialFocus
                                                        className="shadow-sm transition-all duration-200 hover:shadow focus:shadow-md"
                                                    />
                                                </PopoverContent>
...

                        {
                            form.formState.errors.root && (
                                ...
                                >
                                    <p className="text-sm text-rose-600 mt-2">Please select a priority and status</p>
                                </motion.div>
                            )
                        }
                        {/* Submit Button */}
